...
public class Design : MonoBehaviour
{
    ...
    //private List<Vector3> ResamplePoints(Vector3[] originalPoints, float targetPointDistance = 0.005f)
    //{
    //    List<Vector3> resampledPoints = new List<Vector3>();

    //    // Check if there are enough points to resample
    //    if (originalPoints.Length < 2)
    //    {
    //        resampledPoints.AddRange(originalPoints);
    //        return resampledPoints;
    //    }

    //    // Calculate the total distance along the original points
    //    float totalDistance = 0f;
    //    List<float> distances = new List<float> { 0f };
    //    for (int i = 1; i < originalPoints.Length; i++)
    //    {
    //        float distance = Vector3.Distance(originalPoints[i - 1], originalPoints[i]);
    //        totalDistance += distance;
    //        distances.Add(totalDistance);
    //    }

    //    // Determine the number of samples based on the target point distance
    //    int numSamples = Mathf.Max(2, Mathf.RoundToInt(totalDistance / targetPointDistance));

    //    // Generate evenly spaced points along this total distance
    //    float segmentLength = totalDistance / (numSamples - 1);
    //    for (int i = 0; i < numSamples; i++)
    //    {
    //        float targetDistance = i * segmentLength;
    //        resampledPoints.Add(InterpolateAlongCurve(originalPoints, distances, targetDistance));
    //    }

    //    return resampledPoints;
    //}

    //private Vector3 InterpolateAlongCurve(Vector3[] points, List<float> distances, float targetDistance)
    //{
    //    // Handle the case where there's only one point
    //    if (points.Length < 2) return points[0];

    //    // Cap targetDistance within the range of distances
    //    targetDistance = Mathf.Clamp(targetDistance, distances[0], distances[distances.Count - 1]);
    ...
    //    // If segmentIndex is the last index, return the last point
    //    if (segmentIndex >= distances.Count - 1)
    //    {
    //        return points[points.Length - 1];
    //    }

    //    // Proceed with interpolation for valid segmentIndex
    //    float segmentStartDist = distances[segmentIndex];
    //    float segmentEndDist = distances[segmentIndex + 1];
    //    Vector3 start = points[segmentIndex];
    //    Vector3 end = points[segmentIndex + 1];
